...
export class TendenciesComponent implements OnInit {
  ...
  private tendenciesObs$!: Subscription;

  constructor(private router: Router, private daoCatalogue: DAOCatalogueService) { }

  // *** ANGULAR ***
  ngOnInit(): void {
    this.tendenciesObs$ = this.daoCatalogue.catalogue$.subscribe({
      next: data => {
    ...
    this.tendenciesObs$.unsubscribe();
  }
  ...
  onTendenciesClick(e: MouseEvent) {
    ...
    this.router.navigate(['/', 'medio'], { queryParams: { id: elem.id.split('-')[1] } });
  }
